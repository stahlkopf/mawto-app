...
var List = React.createClass({
  render: function() {
    var listItems = this.props.post.summarylong.map(function(item) {
      return <li>{item}</li>;
    });
...
module.exports = React.createClass({
  ...
  renderPostText: function(){
    var endIndex = this.props.post.summarylong.length;
    console.log(this.props.post.summarylong);
    var sumtext = this.props.post.summarylong.map(function(atoms, idx) {
      if (idx == endIndex - 1) {
        return null;
      } else {
        return <li>{atoms}</li>;
  ...
  renderPostText2: function(){
    if(!this.props.post.summarylong){
      return null;
    }
    return(
      <Text style={styles.headerPostText}>
        {this.fixPostText(this.props.post.summarylong)}
      </Text>
    );
  },
